...
final class CartController extends AbstractController
{
    ...
    #[Route('/cart/courses/{id}', name: 'app_cart_courses', methods: ['GET'])]
    public function courses(int $id, EntityManagerInterface $entityManager): Response
    {
        if ($this->getUser() === null) {
            return $this->redirectToRoute('app_login');
        }

        $selectedCourse = $entityManager->getRepository(Item::class)->find($id);
        if (!$selectedCourse) {
            $this->addFlash('error', 'Course not found');
            return $this->redirectToRoute('app_order_singleC');
        }

        $modules = $entityManager->getRepository(LesonModule::class)->findBy(['leson' => $selectedCourse]);
        $subModules = $entityManager->getRepository(SubModule::class)->findBy(['module' => $modules]);

        return $this->render('cart/course.html.twig', [
            'modules' => $modules,
